...
export default function EditKerjasama(params) {
  ...
  useEffect(() => {
    async function getData() {
      ...
      const dataKerjasama = await getKerjasamaById(params.match.params.id);
      setPartner(dataKerjasama.data.partner);
      setProgramStudi(dataKerjasama.data.program_studi.id);
      setNegara(dataKerjasama.data.negara.id);
      setJenisPartner(dataKerjasama.data.jenis_partner.id);
      setJenisDokumen(dataKerjasama.data.jenis_dokumen.id);
      setBentukKegiatan(dataKerjasama.data.bentuk_kegiatan.id);
      setTanggalAwal(dataKerjasama.data.tanggal_awal);
      setTanggalAkhir(dataKerjasama.data.tanggal_akhir);
      setStatus(dataKerjasama.data.status);
      setIsLoading(false);
    }
    getData();
  }, []);
